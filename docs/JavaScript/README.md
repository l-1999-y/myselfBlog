...
为了避免相互影响，为了减少连环崩溃的几率，当启动浏览器后，浏览器会自动启动多个进程。
...
## async await Promise
async/await是ES2017中新增的异步编程语法糖，用于简化Promise的使用。async/await本质上是基于Promise实现的，它可以让异步代码看起来像同步代码，使得代码的可读性和可维护性更高。

async函数返回一个Promise对象，其中包含await表达式的返回值。当调用async函数时，函数内部的代码会被异步执行，直到遇到第一个await表达式。在遇到await表达式时，async函数会暂停执行，等待await表达式的Promise对象状态改变后再继续执行。

与Promise相比，async/await更加直观易懂，可以更好地处理异步操作。同时，async/await还可以使用try/catch结构来处理异步操作中的错误，使得代码更加简洁易读。
